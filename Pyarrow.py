...
def perform_operations(input_dir="bank_data_joins"):
    ...
    # Load the generated data with Arrow and convert to pandas
    print("\nLoading data with Arrow and converting to pandas...")
    
    # Read Parquet files via Arrow
    accounts_arrow = pq.read_table(f"{input_dir}/accounts.parquet")
    merchants_arrow = pq.read_table(f"{input_dir}/merchants.parquet")
    transactions_arrow = pq.read_table(f"{input_dir}/transactions.parquet")
    
    # Convert to pandas
    accounts_df = accounts_arrow.to_pandas()
    merchants_df = merchants_arrow.to_pandas()
    transactions_df = transactions_arrow.to_pandas()
    
    # Free Arrow tables from memory
    del accounts_arrow, merchants_arrow, transactions_arrow
    gc.collect()

    # Memory after loading
    loading_memory = get_memory_usage()
    loading_memory_diff = loading_memory - initial_memory
    print(f"Memory used for loading via Arrow to pandas: {loading_memory_diff:.2f} MB")
    ...
    arrow_table = pa.Table.from_pandas(grouped_df)
    pq.write_table(arrow_table, f"{input_dir}/grouped_data_pandas.parquet")
    
    # Save as CSV directly
    grouped_df.to_csv(f"{input_dir}/grouped_data_pandas.csv", index=False)
